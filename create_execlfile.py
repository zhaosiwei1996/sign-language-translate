...
def worker(wordlist):
    ...
    for resp in query:
        # logging.info('wordlist:' + wordlist + ':' + str(resp))
        landmarks.append(resp)
    df = pd.DataFrame(landmarks)
    df.to_excel('./npyfile/2024head-data/{}.xlsx'.format(wordrep))
    logging.info('wordlist:' + wordlist + ':' + str(landmarks))
    mysql_tool.close()
